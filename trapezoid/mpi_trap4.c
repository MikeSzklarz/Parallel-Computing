...
double Trap(double left_endpt, double right_endpt, long long int trap_count,
            double base_len);
...
int main(int argc, char* argv[]) {
    ...
    // checking if usage is correct based on amount of passed commandline arguments
    if (my_rank == 0) {
        if (argc != 4) {
            printf("Usage: %s a b n\n", argv[0]);
            // MPI_Abort(MPI_COMM_WORLD, 1); This doesnt work but useful to learn about
        }
    }

    // parsing cmndline args
    // atoll is to long long int
    a = atof(argv[1]);
    b = atof(argv[2]);
    n = atoll(argv[3]);

    // testing how long it takes with different -n parameter
    ...
    if (my_rank == 0) {
        ...
        printf("of the integral from %f to %f = %.30e\n", a, b, total_int);
